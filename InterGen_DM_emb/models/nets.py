...
class InterDenoiser(nn.Module):
    def __init__(self,
        ...
        self.class_embedding = nn.Embedding(10, self.latent_dim)
    ...
    def forward(self, x, timesteps, mask=None, cond=None, class_num=None):
        """
        x: B, T, D
        """
        ...
        if mask is not None:
            mask = mask[...,0]
        # 
        #print("cond.shape: ", cond.shape) # B, 768
        #print("class_num: ", class_num) # 1
        if isinstance(class_num, np.ndarray):
            class_num = torch.tensor(class_num, device=cond.device)  # cond와 같은 디바이스로
            
        class_emb = self.class_embedding(class_num)  # (B, D)
        #class_emb = class_emb.unsqueeze(1).expand(B, -1)  # (B, T, D)
        emb = self.embed_timestep(timesteps) + self.text_embed(cond)  # (B, T, D)
        # print("emb.shape: ", emb.shape)
        # print("class_emb.shape: ", class_emb.shape)
        emb = emb + class_emb  # (B, T, D)
...
class InterDiffusion(nn.Module):
    ...
    def forward(self, batch):
        ...
        if not hasattr(self, "pca"):
            self.pca = joblib.load("preprocess_clustering/pca_model.pkl")  # 사전 학습된 PCA
            self.cluster_centers = np.load("preprocess_clustering/kmeans_cluster_centers.npy")  # shape: (10, 20)
        
        cond_np = cond.detach().cpu().numpy()  # (B, 768)
        cond_pca = self.pca.transform(cond_np)  # (B, 20)

        # 3. 거리 계산 → 각 샘플마다 가장 가까운 클러스터 인덱스
        distances = euclidean_distances(cond_pca, self.cluster_centers)  # (B, 10)
        cluster_ids = np.argmin(distances, axis=1)  # (B,)
        c_n = cluster_ids
