...
class EvaluatorModelWrapper(object):
    ...
    def get_co_embeddings(self, batch_data):
        with torch.no_grad():
            ...
            # motion1[..., 22*3:22*6] = 0
            # motion2[..., 22*3:22*6] = 0 #Rotation 무효화
            motions = torch.cat([motion1, motion2], dim=-1)
            motions = motions.detach().to(self.device).float()
            
            #print("motion1.shape: ", motion1.shape) # B, F, C
            #print("motion2.shape: ", motion2.shape) # B, F ,C
            #print("motions.shape: ", motions.shape) # B, F, 2*C  C = 262
