...
class InterHumanDataset(data.Dataset):
    def __init__(self, opt):
        ...
        cluster_data = np.load("preprocess_clustering/train_500_index_to_cluster_n=15.npy", allow_pickle=True).item()
        # 클러스터 딕셔너리 생성 (각 인덱스에 해당하는 클러스터 번호 저장)
        self.cluster_dict = {key: value for key, value in cluster_data.items()}
        ...
        if self.opt.MODE == "train":
            try:
                data_list = open(os.path.join(opt.DATA_ROOT, "train_500.txt"), "r").readlines()
            except Exception as e:
                print(e)
            ...
            try:
                data_list = open(os.path.join(opt.DATA_ROOT, "test_200.txt"), "r").readlines()
            except Exception as e:
                print(e)
    ...
    def __getitem__(self, item):
        ...
        if isinstance(name, str) and name.isdigit():  # name이 문자열로 된 숫자인 경우
            class_idx = int(name)  # 숫자로 변환
        elif isinstance(name, str) and "_swap" in name:  # name이 숫자_swap 형식인 경우
            class_idx = int(name.split('_')[0])  # '_swap' 앞부분만 추출하고 숫자로 변환
        else:
            raise ValueError(f"Unexpected name format: {name}")

        #print(f"Class index for {name}: {class_idx}")
       

        
        class_num = self.cluster_dict.get(class_idx, 'Unknown class')
        ...
        return name, text, gt_motion1, gt_motion2, gt_length, class_num
