    ...
    def __init__(self, model, dataset, device, mm_num_samples, mm_num_repeats):
        ...
        with torch.no_grad():
            for i, data in tqdm(enumerate(dataloader)):
                name, text, motion1, motion2, motion_lens, class_num = data
                batch = {}
                if i in mm_idxs:
...
class EvaluatorModelWrapper(object):
    ...
    def get_co_embeddings(self, batch_data):
        with torch.no_grad():
            if len(batch_data) == 6:
                name, text, motion1, motion2, motion_lens, class_num = batch_data
            else:
                name, text, motion1, motion2, motion_lens = batch_data
                class_num = None  # 혹은 default 처리
            #name, text, motion1, motion2, motion_lens, class_num = batch_data
